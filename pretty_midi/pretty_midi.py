...
class PrettyMIDI(object):
    ...
    def __init__(self, midi_file=None, resolution=220, initial_tempo=120.0):
        ...
        if midi_file is not None:
            ...
            if max_tick > MAX_TICK:
                raise ValueError(
                    (
                        "MIDI file has a largest tick of {},"
        ...
        for event in midi_data.tracks[0]:
            if event.type == "key_signature":
                key_obj = KeySignature(
                    key_name_to_key_number(event.key), self.__tick_to_time[event.time]
                )
                self.key_signature_changes.append(key_obj)

            elif event.type == "time_signature":
                ts_obj = TimeSignature(
                    event.numerator, event.denominator, self.__tick_to_time[event.time]
                )
        ...
        for track in midi_data.tracks:
            ...
            for event in track:
                if event.type == "lyrics":
                    ...
                    text_events.append(
                        Text(event.text, self.__tick_to_time[event.time])
                    )
    ...
    def _update_tick_to_time(self, max_tick):
        ...
        for (start_tick, tick_scale), (end_tick, _) in zip(
            self._tick_scales[:-1], self._tick_scales[1:]
        ):
            # Convert ticks in this interval to times
            ticks = np.arange(end_tick - start_tick + 1)
            self.__tick_to_time[start_tick : end_tick + 1] = (
                last_end_time + tick_scale * ticks
            )
            ...
            for event in track:
                ...
                # Store control changes
                    ...
                    control_change = ControlChange(
                        event.control, event.value, self.__tick_to_time[event.time]
                    )
                    # Get the program for the current inst
                    program = current_instrument[event.channel]
                    # Retrieve the Instrument instance for the current inst
    ...
    def estimate_tempi(self):
        ...
        for interval in ioi:
            ...
            if (np.abs(clusters - interval) < 0.025).any():
                ...
                # Update cluster mean
                clusters[k] = (cluster_counts[k] * clusters[k] + interval) / (
                    cluster_counts[k] + 1
                )
    ...
    def estimate_tempo(self):
        ...
        if tempi.size == 0:
            raise ValueError(
                "Can't provide a global tempo estimate when there"
    ...
    def get_beats(self, start_time=0.0):
        ...
        while (
            tempo_idx < tempo_change_times.shape[0] - 1
            and beats[-1] > tempo_change_times[tempo_idx + 1]
        ):
            tempo_idx += 1
        ...
        while (
            ts_idx < len(self.time_signature_changes) - 1
            and beats[-1] >= self.time_signature_changes[ts_idx + 1].time
        ):
            ts_idx += 1
        ...
        while beats[-1] < end_time:
            ...
            if (
                tempo_idx < tempo_change_times.shape[0] - 1
                and next_beat > tempo_change_times[tempo_idx + 1]
            ):
                ...
                while (
                    tempo_idx < tempo_change_times.shape[0] - 1
                    and next_beat + beat_remaining * 60.0 / bpm
                    >= tempo_change_times[tempo_idx + 1]
                ):
                    # Compute the amount the beat location overshoots
                    overshot_ratio = (tempo_change_times[tempo_idx + 1] - next_beat) / (
                        60.0 / bpm
                    )
            ...
            if self.time_signature_changes and ts_idx == 0:
                ...
                if current_ts_time > beats[-1] and gt_or_close(
    ...
    def estimate_beat_start(self, candidates=10, tolerance=0.025):
        ...
        while (
            len(beat_candidates) <= candidates
            and len(beat_candidates) <= len(note_list)
            and onset_index < len(note_list)
        ):
            # Make sure we are using a new start location
            if (
                onset_index == 0
                or np.abs(
                    note_list[onset_index - 1].start - note_list[onset_index].start
                )
                > 0.001
            ):
    ...
    def get_downbeats(self, start_time=0.0):
        ...
        for start_ts, end_ts in zip(time_signatures[:-1], time_signatures[1:]):
            ...
            if start_ts.numerator % 3 == 0 and start_ts.numerator != 3:
                downbeats.append(
                    beats[start_beat_idx : end_beat_idx : (start_ts.numerator // 3)]
                )
            else:
                downbeats.append(
    ...
    def get_control_change_roll(self, fs=100, cc_threshold=64, times=None):
        """Compute a control change roll vector of the MIDI data.
        ...
        Returns
        -------
        pedal_roll : np.ndarray, shape=(128, times.shape[0])
            Pedal roll of MIDI data, flattened across instruments.
        ...
        pedal_roll = np.zeros((128, np.max([p.shape[1] for p in pedal_rolls])))
        # Sum each piano roll into the aggregate piano roll
        for roll in pedal_rolls:
            pedal_roll[:, : roll.shape[1]] += roll
        return pedal_roll
    ...
    def get_piano_onset_roll(self, fs=100, times=None, onset_length=0.064):
        """Compute a piano roll matrix of the MIDI data.
        ...
        # Get piano rolls for each instrument
        piano_rolls = [
            i.get_piano_onset_roll(fs=fs, times=times, onset_length=onset_length)
            for i in self.instruments
        ]
        # Allocate piano roll,
        # number of columns is max of # of columns in all piano rolls
    ...
    def get_piano_offset_roll(
        self, fs=100, times=None, pedal_threshold=64, offset_length=0.064
    ):
        """Compute a piano roll matrix of the MIDI data.
    ...
    def get_pitch_class_histogram(
        self, use_duration=False, use_velocity=False, normalize=True
    ):
    ...
    def time_to_tick(self, time):
        ...
        if tick == len(self.__tick_to_time):
            ...
            # Re-round/quantize
            if type(tick) == np.ndarray or type(tick) == list:
                tick = tick[0]
            return int(round(tick))
        # If the tick is not 0 and the previous ticktime in a is closer to time
        if tick and (
            math.fabs(time - self.__tick_to_time[tick - 1])
    ...
    def adjust_times(self, original_times, new_times):
        ...
        note_ons = np.array(
            [note.start for instrument in self.instruments for note in instrument.notes]
        )
        adjusted_note_ons = np.interp(note_ons, original_times, new_times)
        # Same for note-offs
        note_offs = np.array(
            [note.end for instrument in self.instruments for note in instrument.notes]
        )
        adjusted_note_offs = np.interp(note_offs, original_times, new_times)
        # Correct notes
        for n, note in enumerate(
            [note for instrument in self.instruments for note in instrument.notes]
        ):
        ...
        if adjusted_downbeats.size > 0:
            ...
            ts_changes_before_downbeat = [
                t
                for t in self.time_signature_changes
        ...
        original_times = [
            self.__tick_to_time[self.time_to_tick(time)] for time in original_times
        ]
        ...
        non_repeats = [0] + [
            n
            for n in range(1, len(new_times))
            if new_times[n - 1] != new_times[n]
            and original_times[n - 1] != original_times[n]
        ]
        ...
        tempo_idx = 0
        while (
            tempo_idx + 1 < len(tempo_changes)
            and original_times[0] >= tempo_change_times[tempo_idx + 1]
        ):
            tempo_idx += 1
        ...
        for start_time, end_time, speed_scale in zip(
            ...
            while (
                tempo_idx + 1 < len(tempo_changes)
                and start_time <= tempo_change_times[tempo_idx + 1]
                and end_time > tempo_change_times[tempo_idx + 1]
            ):
                tempo_idx += 1
    ...
    def write(self, filename):
        ...
        def event_compare(event1, event2):
            ...
            secondary_sort = {
                ...
                "note_on": lambda e: ((9 * 256 * 256) + (e.note * 256) + e.velocity),
            ...
            if (
                ...
                return secondary_sort[event1.type](event1) - secondary_sort[
                    event2.type
                ](event2)
        ...
        for ks in self.key_signature_changes:
            timing_track.append(
                mido.MetaMessage(
                    "key_signature",
                    time=self.time_to_tick(ks.time),
        ...
        for l in self.lyrics:
            timing_track.append(
                mido.MetaMessage("lyrics", time=self.time_to_tick(l.time), text=l.text)
            )
        # Add text events
        ...
        timing_track.append(
            mido.MetaMessage("end_of_track", time=timing_track[-1].time + 1)
        )
        ...
        for n, instrument in enumerate(self.instruments):
            ...
            for note in instrument.notes:
                ...
                track.append(
                    mido.Message(
                        "note_on",
                        time=self.time_to_tick(note.start),
                        channel=channel,
                        note=note.pitch,
                ...
                track.append(
                    mido.Message(
                        "note_on",
                        time=self.time_to_tick(note.end),
                        channel=channel,
                        note=note.pitch,
                        velocity=0,
                    )
            ...
            for bend in instrument.pitch_bends:
                track.append(
                    mido.Message(
                        "pitchwheel",
                        time=self.time_to_tick(bend.time),
                        channel=channel,
                        pitch=bend.pitch,
                    )
                )
